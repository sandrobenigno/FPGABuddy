...
### Este projeto está relacionado ao meu core de Atari 2600: [A2600NanoSAN](http://github.com/sandrobenigno/A2600NanoSAN)
---
...
* **`STATE_SELECIONANDO`**:
  ...
  * **Menu de Sistema / Configuração**: Pressionar o encoder por 1s (clique longo) enquanto estiver no grid de letras abre o menu de sistema (subestado `SUBSTATE_DB_MENU`), oferecendo as opções **Atualizar Jogos** (faz o scan físico no SD, atualiza o campo `sd` de cada registro no arquivo `/roms.bin` e exibe o progresso em tempo real), **Idioma** (permite alternar o idioma do sistema em tempo real entre Português e Inglês, salvando a preferência no arquivo `/fpgabuddy.cfg` no cartão SD) e **Voltar**.
...
* `resources/FPGA_adds_mods/ADD/db9_to_spi_san.v`: Módulo Master SPI para o adaptador de gamepad DB9-para-SPI no Tang Nano 20k.
* `resources/FPGA_adds_mods/MOD/mcu_spi.v`: Módulo deserializador SPI na FPGA modificado com saída Tri-state na linha MISO.
* `resources/FPGA_adds_mods/MOD/a2600_top_tn20k_san.vhd`: Módulo principal VHDL do core A2600Nano modificado para integrar o carregador SPI e periféricos customizados.
