...
bool db_init(void) {
    int rc = sqlite3_open("/jogos.db", &db);
    if (rc != SQLITE_OK) {
        printf("[DB] Falha ao abrir o SQLite: %s\n", sqlite3_errmsg(db));
        sqlite3_close(db);
        db = NULL;
        return false;
    }
    printf("[DB] Banco SQLite aberto com sucesso.\n");
    return true;
}

bool db_get_available_letters(bool letras_disponiveis[26]) {
    if (!db) return false;
    memset(letras_disponiveis, 0, 26 * sizeof(bool));

    sqlite3_stmt* stmt;
    const char* sql = "SELECT DISTINCT letra FROM jogos WHERE letra >= 'A' AND letra <= 'Z'";
...
bool db_fetch_games_by_letter(char letra, GameInfo** list, int* count) {
    if (!db || !list || !count) return false;
    
    // 1. Limpa cache antigo se aplicável
    *list = NULL;
    *count = 0;

    // 2. Conta quantidade de registros
    sqlite3_stmt* stmt;
    const char* count_sql = "SELECT COUNT(*) FROM jogos WHERE letra = ?";
    if (sqlite3_prepare_v2(db, count_sql, -1, &stmt, NULL) != SQLITE_OK) {
        return false;
    }
    char letra_str[2] = {letra, '\0'};
    sqlite3_bind_text(stmt, 1, letra_str, -1, SQLITE_STATIC);
    ...
    // 3. Aloca memória na heap para a letra
    *list = (GameInfo*)malloc((*count) * sizeof(GameInfo));
    if (!*list) {
        *count = 0;
        return false; // Heap Exhaustion
    }

    // 4. Popula o cache dinâmico
    const char* select_sql = "SELECT md5, nome, tamanho FROM jogos WHERE letra = ? ORDER BY nome";
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW && idx < *count) {
        const unsigned char* md5_val = sqlite3_column_text(stmt, 0);
        const unsigned char* nome_val = sqlite3_column_text(stmt, 1);
        uint32_t tam = sqlite3_column_int(stmt, 2);

        if (md5_val) {
            strncpy((*list)[idx].md5, (const char*)md5_val, 32);
            (*list)[idx].md5[32] = '\0';
        }
        if (nome_val) {
            strncpy((*list)[idx].nome, (const char*)nome_val, 63);
            (*list)[idx].nome[63] = '\0';
        }
        (*list)[idx].tamanho = tam;
        idx++;
    }
    sqlite3_finalize(stmt);
    return true;
...
bool db_check_game_exists(const char* md5) {
    if (!db) return false;
    sqlite3_stmt* stmt;
    const char* sql = "SELECT COUNT(*) FROM jogos WHERE md5 = ?";
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) {
        return false;
    }
    sqlite3_bind_text(stmt, 1, md5, -1, SQLITE_STATIC);
    int cnt = 0;
    if (sqlite3_step(stmt) == SQLITE_ROW) {
